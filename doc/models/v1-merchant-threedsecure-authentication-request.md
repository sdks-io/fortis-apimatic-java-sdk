...
| `PreferredProtocolVersion` | [`PreferredProtocolVersionEnum`](../../doc/models/preferred-protocol-version-enum.md) | Optional | Specifies the preferred version of 3D Secure protocol to be utilized while executing 3D Secure authentication. 3DS Server initiates an authentication request with the preferred version and if this version is not supported by other 3D Secure components, it falls back to the next supported version(s) and continues authentication.<br>If the preferred version is enforced by setting  #enforcePreferredProtocolVersion flag, but this version is not supported by one of the 3D Secure components, 3DS Server does not initiate an authentication and provides corresponding error message to the customer.<br>If no value is provided, EMV 3DS 2.1.0 version will be used by default.<br><br>> 2.1.0 - prefer authentication with 2.1.0 version<br>> <br>> 2.2.0 - prefer authentication with 2.2.0 version<br>> <br>> 2.3.1 - prefer authentication with 2.3.1 version | PreferredProtocolVersionEnum getPreferredProtocolVersion() | setPreferredProtocolVersion(PreferredProtocolVersionEnum preferredProtocolVersion) |
...
| `DeviceChannel` | [`DeviceChannelEnum`](../../doc/models/device-channel-enum.md) | Required | Indicates the type of channel interface being used to initiate the transaction.<br><br>> 02 - Browser (BRW)<br>> <br>> 03 - 3DS Requestor Initiated (3RI) | DeviceChannelEnum getDeviceChannel() | setDeviceChannel(DeviceChannelEnum deviceChannel) |
| `MessageCategory` | [`MessageCategoryEnum`](../../doc/models/message-category-enum.md) | Required | Identifies the category of the message for a specific use case.<br><br>> 01 - Payment Authentication (PA)<br>> <br>> 02 - Non-Payment Authentication (NPA)<br>> <br>> 80 through 99 - PS Specific Values<br>> <br>> 80 - MasterCard Identity Check Insights<br>> <br>> 85 - MasterCard Identity Check, Production Validation PA<br>> <br>> 86 - MasterCard Identity Check, Production Validation NPA | MessageCategoryEnum getMessageCategory() | setMessageCategory(MessageCategoryEnum messageCategory) |
...
| `ThreeDsRequestor` | [`ThreeDsRequestor`](../../doc/models/three-ds-requestor.md) | Required | Contains information for the 3DS Requestor. | ThreeDsRequestor getThreeDsRequestor() | setThreeDsRequestor(ThreeDsRequestor threeDsRequestor) |
...
| `CardholderAccount` | [`CardholderAccount`](../../doc/models/cardholder-account.md) | Required | Contains information for the Cardholder Account. | CardholderAccount getCardholderAccount() | setCardholderAccount(CardholderAccount cardholderAccount) |
| `Cardholder` | [`Cardholder`](../../doc/models/cardholder.md) | Optional | Contains information for the Cardholder. This field is required unless market or regional mandate restricts sending this information. | Cardholder getCardholder() | setCardholder(Cardholder cardholder) |
| `Purchase` | [`Purchase`](../../doc/models/purchase.md) | Optional | Contains purchase information | Purchase getPurchase() | setPurchase(Purchase purchase) |
| `BroadInfo` | [`BroadInfo`](../../doc/models/broad-info.md) | Optional | Until EMV 3DS 2.2.0:<br><br>Unstructured information sent between the 3DS Server, the DS and the ACS.<br><br>This field is not required to be filled by the Requestor and the requirements for the presence of this field are DS specific.<br><br>Starting from EMV 3DS 2.3.1:<br><br>Structured information sent between the 3DS Server, the DS and the ACS. 2.3.1 structure is defined below. Accepted value length is maximum 4096 characters. This field is optional. | BroadInfo getBroadInfo() | setBroadInfo(BroadInfo broadInfo) |
