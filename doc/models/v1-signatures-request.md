
# V1 Signatures Request

## Structure

`V1SignaturesRequest`

## Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `Signature` | `String` | Required | Signature | String getSignature() | setSignature(String signature) |
| `Resource` | [`ResourceEnum`](../../doc/models/resource-enum.md) | Required | Resource<br><br>> Recurring, Transaction, AccountVault, DeviceTerm | ResourceEnum getResource() | setResource(ResourceEnum resource) |
| `ResourceId` | `String` | Required | Resource ID<br><br>**Constraints**: *Pattern*: `^(([0-9a-fA-F\-]{24,36})\|(([0-9a-fA-F]{8})-(([0-9a-fA-F]{4}\-){3})([0-9a-fA-F]{12})))$` | String getResourceId() | setResourceId(String resourceId) |

## Example (as JSON)

```json
{
  "signature": "",
  "resource": "Transaction",
  "resource_id": "11e95f8ec39de8fbdb0a4f1a"
}
```

