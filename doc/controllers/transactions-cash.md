...
```java
TransactionsCashController transactionsCashController = client.getTransactionsCashController();
```
...
```java
V1TransactionsCashRefundRequest body = new V1TransactionsCashRefundRequest.Builder(
    1
)
...
transactionsCashController.cashRefundAsync(body).thenAccept(result -> {
...
```java
V1TransactionsCashSaleRequest body = new V1TransactionsCashSaleRequest.Builder(
    1
)
.checkinDate("2021-12-01")
.checkoutDate("2021-12-01")
...
transactionsCashController.cashSaleAsync(body).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
