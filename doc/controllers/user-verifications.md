...
```java
UserVerificationsController userVerificationsController = client.getUserVerificationsController();
```
...
```java
CompletableFuture<ApiResponse<ResponseUserVerification>> getuserverificationAsync(
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`ResponseUserVerification`](../../doc/models/response-user-verification.md).
...
userVerificationsController.getuserverificationAsync(userVerificationId).thenAccept(result -> {
...
```java
CompletableFuture<ApiResponse<ResponseUserVerificationsCollection>> listuserverificationsAsync(
    final Page1 page,
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`ResponseUserVerificationsCollection`](../../doc/models/response-user-verifications-collection.md).
...
userVerificationsController.listuserverificationsAsync(page, order, filterBy, null, null, null, null).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
