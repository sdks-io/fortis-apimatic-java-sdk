...
Any user with the available permissions can perform a request to manually settle a credit card batch. This is done by calling the batches endpoint with the batch_id to manually settle the batch. By default the gateway operates in auto settlement mode.
...
batchesController.listAllBatchesAsync(page, order, filterBy, null, null, null, null).thenAccept(result -> {
