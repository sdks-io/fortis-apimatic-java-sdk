...
Contacts are used to identify customers and allows integrators to drill down each contact’s transaction history as it relates to tokens, recurring and one-time payments.
...
contactsController.contactsSearchAsync(locationId, page, null, null).thenAccept(result -> {
...
```java
V1ContactsRequest body = new V1ContactsRequest.Builder(
...
```java
String contactId = "11e95f8ec39de8fbdb0a4f1a";
contactsController.viewSingleContactAsync(contactId, null, null).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
String contactId = "11e95f8ec39de8fbdb0a4f1a";
V1ContactsRequest1 body = new V1ContactsRequest1.Builder()
...
contactsController.updateContactAsync(contactId, body, null).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
