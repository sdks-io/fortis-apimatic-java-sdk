/*
 * FortisAPILib
 *
 * This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
 */

package tech.fortis.sandbox.api.models;

import com.fasterxml.jackson.annotation.JsonGetter;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonSetter;
import io.apimatic.core.types.BaseModel;

/**
 * This is a model class for ResponseDeviceTerm type.
 */
public class ResponseDeviceTerm
        extends BaseModel {
    private Type22Enum type;
    private Data6 data;

    /**
     * Default constructor.
     */
    public ResponseDeviceTerm() {
        type = Type22Enum.DEVICETERM;
    }

    /**
     * Initialization constructor.
     * @param  type  Type22Enum value for type.
     * @param  data  Data6 value for data.
     */
    public ResponseDeviceTerm(
            Type22Enum type,
            Data6 data) {
        this.type = type;
        this.data = data;
    }

    /**
     * Getter for Type.
     * Resource Type
     * @return Returns the Type22Enum
     */
    @JsonGetter("type")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public Type22Enum getType() {
        return type;
    }

    /**
     * Setter for Type.
     * Resource Type
     * @param type Value for Type22Enum
     */
    @JsonSetter("type")
    public void setType(Type22Enum type) {
        this.type = type;
    }

    /**
     * Getter for Data.
     * @return Returns the Data6
     */
    @JsonGetter("data")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public Data6 getData() {
        return data;
    }

    /**
     * Setter for Data.
     * @param data Value for Data6
     */
    @JsonSetter("data")
    public void setData(Data6 data) {
        this.data = data;
    }

    /**
     * Converts this ResponseDeviceTerm into string format.
     * @return String representation of this class
     */
    @Override
    public String toString() {
        return "ResponseDeviceTerm [" + "type=" + type + ", data=" + data
                + ", additionalProperties=" + getAdditionalProperties() + "]";
    }

    /**
     * Builds a new {@link ResponseDeviceTerm.Builder} object.
     * Creates the instance with the state of the current model.
     * @return a new {@link ResponseDeviceTerm.Builder} object
     */
    public Builder toBuilder() {
        Builder builder = new Builder()
                .type(getType())
                .data(getData());
        return builder;
    }

    /**
     * Class to build instances of {@link ResponseDeviceTerm}.
     */
    public static class Builder {
        private Type22Enum type = Type22Enum.DEVICETERM;
        private Data6 data;



        /**
         * Setter for type.
         * @param  type  Type22Enum value for type.
         * @return Builder
         */
        public Builder type(Type22Enum type) {
            this.type = type;
            return this;
        }

        /**
         * Setter for data.
         * @param  data  Data6 value for data.
         * @return Builder
         */
        public Builder data(Data6 data) {
            this.data = data;
            return this;
        }

        /**
         * Builds a new {@link ResponseDeviceTerm} object using the set fields.
         * @return {@link ResponseDeviceTerm}
         */
        public ResponseDeviceTerm build() {
            return new ResponseDeviceTerm(type, data);
        }
    }
}
